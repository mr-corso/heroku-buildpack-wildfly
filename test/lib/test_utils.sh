...
# Additional hooks to be overriden in test files because the
# functions 'oneTimeSetUp' and 'oneTimeTearDown' are already
# used. The TEST_SUITE_CACHE directory can be used for caching
# test resources.
setUpOnce() {
    :
}
...
assertAppDetected() {
    local expectedAppType="${1?"Must provide app type"}"
...
_assertContains() {
    ...
    if [ $# -eq 5 ]; then
        ...
        fail "Expected 4 or 5 parameters; Received $# parameters"
    fi
...
assertFileMD5() {
    ...
    local md5_cmd expected_md5_cmd_output
